...
class Grid:
    ...
    def is_solved(self):
        ...
        for i in range(self._width):
            for j in range(self._width):
                if len(self._cells[i][j]) > 1 or not self.is_value_consistent(
                    self._cells[i][j], i, j
                ):
                    return False
        return True
...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                domain_length = len(grid.get_cells()[i][j])
                if domain_length == 1:
                    continue
                if domain_length < min_domain:
                    min_domain = domain_length
                    min_var = (i, j)
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q) > 0:
            var = Q.pop()
            asg1, f1 = self.remove_domain_row(grid, var[0], var[1])
            asg2, f2 = self.remove_domain_column(grid, var[0], var[1])
            asg3, f3 = self.remove_domain_unit(grid, var[0], var[1])

            if f1 == True or f2 == True or f3 == True:
                return True
            for i in asg1:
                Q.append(i)
            for j in asg2:
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        # var = select-unassigned-var(A)
        # for d in domain(var):
        #     if d is consistent with A:
        #     copy_A = A.copy()
        #     {var = d} in copy_A
        #     rb = Backtracking(copy_A)
        #     if rb is not failure:
        #         return rb
        # return failure
        ...
        for d in grid.get_cells()[var[0]][var[1]]:
            if grid.is_value_consistent(d, var[0], var[1]):
                ...
                if not ac3.consistency(grid, [(var[0], var[1])]):
                    rb = self.search(copy_grid, var_selector)
                    if rb is not False:
                        return rb
        copy_grid.get_cells()[var[0]][var[1]] = ""
...
mrv = MRV()
var = mrv.select_variable(g)
back = Backtracking()
rb = back.search(g, mrv)
rb.print()
